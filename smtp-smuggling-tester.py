...
if __name__ == '__main__':
    ...
    argument_parser.add_argument("--sender-domain", help="要使用的发件人域。请确保您拥有此域的有效 SPF 记录。",default="check.smtpsmuggling.com")
    ...
    argument_parser.add_argument("--smuggling-identifier", help="用于在接收 SMTP 分析服务器上突出显示的标识符。", default="SMUGGLING")
    argument_parser.add_argument("--setup-check", help="通过发送测试电子邮件检查您的设置是否正常运行。", action="store_true")
    argument_parser.add_argument("--tls", help="强制使用 TLS。（不要忘记更改端口！）", action="store_true")
    argument_parser.add_argument("--starttls", help="强制使用 STARTTLS。（通常需要发送出站电子邮件）", action="store_true")
    argument_parser.add_argument("--debug", help="输出调试信息。", action="store_true")
